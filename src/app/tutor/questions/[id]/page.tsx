  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (questionsData.question) {
        ...
        // Load alternate answers if present
        setAlternateAnswers(answer.alternates || [])
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (answerType === 'multiple_choice') {
      ...
      correctAnswerJson = {
        ...
        tolerance,
        alternates: alternateAnswers.filter(a => a.trim())
      }
    } else {
      correctAnswerJson = { 
        value: answerValue,
              ...
              <div className="border-t border-gray-200 pt-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-gray-500">
                    Accept these as correct in addition to the main answer
                  </span>
                </div>
                {alternateAnswers.length > 0 ? (
                  <div className="space-y-2">
                    {alternateAnswers.map((alt, index) => (
                      <div key={index} className="flex items-center gap-2">
                        <span className="text-sm text-green-600 font-medium">Alt {index + 1}</span>
                        <input
                          type={answerType === 'numeric' ? 'number' : 'text'}
                          value={alt}
                          onChange={(e) => {
                            const newAlts = [...alternateAnswers]
                            newAlts[index] = e.target.value
                            setAlternateAnswers(newAlts)
                          }}
                          step={answerType === 'numeric' ? 'any' : undefined}
                        ...
                        <button
                          type="button"
                          onClick={() => setAlternateAnswers(alternateAnswers.filter((_, i) => i !== index))}
                  ...
                  <p className="text-sm text-gray-500 mb-2">No alternate answers defined.</p>
                )}
                <button
                  type="button"
                  onClick={() => setAlternateAnswers([...alternateAnswers, ''])}
